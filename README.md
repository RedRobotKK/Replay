...
| **Telemetry** | None, and nothing to opt out of. No account, no install-time question, no first-run prompt. **The binary makes no network request except the proxy forwarding your own traffic to the provider you configured**, and two you have to type. `rules --check-prices` fetches a public price database to compare against the compiled table, sends nothing, and installs nothing. `probe --execute` is the one command that originates traffic rather than forwarding it: it sends synthetic, billable requests to your configured provider on your own key, to measure the cache floor. It refuses to run without `--execute`, prints the plan first, and asks before spending. **This sentence said "one" until 2026-09-06 and was wrong from the day `probe` shipped** — the command was built on purpose, and this line and [`SURFACES.md`](docs/SURFACES.md) were not updated with it. There is no submit command and no corpus endpoint: contribution was designed ([ADR-0007](docs/adr/0007-federated-calibration-corpus.md), [ADR-0008](docs/adr/0008-corpus-at-launch.md)) and is not built. [Every outbound surface, including `doctor`'s probe for an already-running proxy](docs/SURFACES.md) |

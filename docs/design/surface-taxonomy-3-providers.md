...
| **Usage merge across frames** | field-wise merge, non-zero wins **[code]** `response.go:155-187` | last frame wins **[code]** `openai_stream.go:77-86` | accumulated when the cumulative advances; an unchanged cumulative is the same usage re-emitted and is not added again **[code]** `codex.go:349-360` | — | — |

...
| **S9 Browser-origin refusal** | `Origin`, `Sec-Fetch-Mode` | on, unconditional (`server.go:428-431`) | either header present → 403 | a web page reaches the proxy via DNS rebinding | No | there is **no `Host` validation**; the whole defence rests on `Origin` being present | None |
| | | | Tested: `TestBrowserOriginAndTokenChecks` (`server_test.go:390`) — **`Sec-Fetch-Mode` branch UNTESTED** | | | | |
...
| **S11 Health exemption** | `/replay/healthz` | on | — | `health` (`server.go:420-423`) calls **neither** `localOnly` nor the token check, so it answers cross-origin and unauthenticated | No | a web page can fingerprint that Replay is running | None. Recorded as a known gap at `docs/SURFACES.md:97` |

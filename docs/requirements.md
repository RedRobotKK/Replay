...
| RE-2 | Calibration: reproduce the provider's reported cache reads and cache writes for every turn of a recorded session before scoring alternatives. **Status:** 99.00% (398/402 turns) across 11 real sessions, every mismatch located and classified; the corpus is one project on one machine, so the 20-session bar is not met. | Match on at least 95 percent of turns across a 20-session corpus; every mismatch is listed with a reason. |

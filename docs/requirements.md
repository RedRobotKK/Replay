...
**Status, 2026-09-07: the binary offers twenty.** This section is left as written because it is the requirement, and the drift from four to twenty is a fact about the build rather than a change of intent — `cost`, `context`, `route`, `trim`, `advise`, `learn`, `probe`, `corpus`, `rules`, `statusline`, `redact`, `doctor`, `codex`, `burn`, `tui` and `version` all grew out of the four rather than replacing them. Two consequences are worth recording here rather than only in the guide. The bare `replay` form now runs the cost report over the transcript root it discovers, so the first thing a new reader sees is a finding rather than this list; and `replay --help` groups and ranks the twenty by value, because a flat list put the specialist commands in front of the ones that answer the question people arrive with. [`guide/commands.md`](guide/commands.md) is the reference, and a test fails the build if it stops covering every command and every flag.
...
| RE-2 | Calibration: reproduce the provider's reported cache reads and cache writes for every turn of a recorded session before scoring alternatives. **Status:** met — the engine reproduces the provider's reported cache reads comfortably above the 95% threshold, and every transcript below it is listed rather than dropped. The rate and the sample size are not quoted in this row. The corpus is this machine's live transcript store and the engine changes under it, so both move, and this row has carried a withdrawn figure twice; [`evidence/`](evidence/README.md) holds the readings, each dated by the run that produced it, and the [2026-09-06 correction](evidence/calibration-corpus-2026-09-06.md) explains why a transcript count is not a session count. The session gate is met several times over; **independence is not, and no session count fixes that**, because every session comes from one machine. | Match on at least 95 percent of turns across a 20-session corpus; every mismatch is listed with a reason. |
...
| MK-3 | The vault persists encrypted at rest with a key from the OS keychain; a daemon restart loses nothing within the retention window. **Status, 2026-09-10:** encrypted at rest under an owner-only key file, whose mode is now verified on every open rather than only at creation. Entries expire after 24h (`--mask-ttl`), so "a restart loses nothing" holds inside the window and not beyond it — and a lapsed secret that is re-sent is restored under the same placeholder. **The keychain is still not integrated, and the reason is structural rather than pending:** reaching macOS's needs `os/exec`, which `TestX402_ExecIsConfinedToTheMutationHarness` keeps out of every ordinary build because it can call anything. Trading that guard for a boundary on one file is recorded as a decision in `RELEASE-CRITERIA.md`, not deferred here. | Restart test mid-session; expiry covered by `internal/masking/eviction_test.go`. |

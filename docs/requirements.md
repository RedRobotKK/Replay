...
| RE-2 | Calibration: reproduce the provider's reported cache reads and cache writes for every turn of a recorded session before scoring alternatives. **Status:** 97.32% (26571/27302 turns) across 1363 real sessions ([corpus](evidence/calibration-corpus-2026-09-05.md)); 429 sessions fall below the 95% threshold and are listed rather than dropped. The rate is lower than the 11-session figure it replaces because the sample is no longer only this repository's own development work. The session count is met; independence is not, because all of them come from one machine. | Match on at least 95 percent of turns across a 20-session corpus; every mismatch is listed with a reason. |

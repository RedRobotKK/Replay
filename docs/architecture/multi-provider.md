...
| 3. A second provider | **Done, and labelled EXPERIMENTAL, UNMASKED.** `/v1/chat/completions` is read, guarded and ledgered, streaming included. First live run on 2026-09-05 against a local Ollama endpoint: passthrough, usage parsing and streaming all correct, and it **found a defect no stub could** (see below). Verified against live DeepSeek on 2026-09-05 across all four surfaces: chat non-streaming, a second call that hits cache, streaming, and the reasoner. Every invariant held. What the label carries is the part none of that covers: **`--mask` does not read this body shape at all**, so secrets on this path leave the machine in clear, and no third implementation of the API has been tried. The proxy prints the label on stderr once per path, unconditionally |

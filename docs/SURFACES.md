...
| `~/.replay/advice.json` | **read and write** | `advise` output, and `replay tui`'s triage screen writes your decisions into it. **Contains raw tool names and file base names** taken from your transcripts. Listed as write-only in the first version: it has been read back on every `advise` run since tracking existed. Schema 2 from 2026-09-23 separates the `decisions` object, which is what you marked, from each suggestion's `status`, which is recomputed from your corpus every run; a file at schema 1 is skipped rather than misread, so its statuses are discarded and nothing in it is promoted into a decision ([ADR-0027](adr/0027-a-reader-decision-is-persisted-apart-from-the-computed-status.md)) | Read |

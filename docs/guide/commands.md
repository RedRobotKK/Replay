...
**Your own currency, beside the dollars.** If your locale names a territory whose currency the
built-in rate table carries, the summary shows the local equivalent next to each figure and states
the rate once underneath. The locale comes from `LC_ALL`, then `LC_MONETARY`, then `LANG`, in that
order, because a developer commonly runs an English interface with a local monetary locale and
reading `LANG` alone would tell that person their money is dollars. A dollar locale sees exactly
what it saw before: no second column, no rate, no note.

The dollars are never replaced, and that is the whole design. The provider bills in US dollars;
your card issuer converts at its own rate on its own settlement date and usually adds a foreign
transaction fee of 1.5 to 3 percent. A figure converted at a reference rate is an indication of
size, not your bill, and the note under the block says so rather than leaving you to know it.

Rates are a dated snapshot of the European Central Bank's daily reference rates, compiled in and
regenerated by `scripts/fetch-rates.sh`, so the provenance of every number is its diff. Nothing
reaches the network at run time. A rate older than 30 days says its age; older than a year and the
conversion stops entirely, because exchange rates move faster than anything this tool measures and
a wrong number in your own currency is more convincing than a wrong number in a foreign one.

Set `REPLAY_FX_<CODE>` to use a rate you trust instead, for example `REPLAY_FX_JPY=152.4`. It wins
over the table and is attributed to you rather than to the ECB. Set to something unusable, it stops
the conversion and says so: a setting that looks applied and is not is worse than no setting.

Thousands separators are Western grouping for every currency. `de_DE` writes 1.234,56 and `hi_IN`
groups as 12,34,567; getting those right needs a locale database this binary will not carry.

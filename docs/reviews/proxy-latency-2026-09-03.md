...
How much time `buffy serve` adds to a request, measured against a local fake provider so the number is the proxy's own overhead and not the network's.

## Method

- Fake provider: a local HTTP server returning a fixed 2KB Messages API response with usage fields, on loopback.
- Request: a 46,252-byte Messages API body (4KB system prompt, 20 tool definitions, 41 messages with 300-line tool results), sent 300 times directly and 300 times through `buffy serve` with the ledger enabled, after 20 warm-up requests each way.
...
| Through `buffy serve` | 44.03ms | 48.14ms |
| **Added by Buffy** | **48µs** | **98µs** |

...
2. **Live policies use only provider-sanctioned mechanisms.** Request parameters (context editing, cache breakpoint placement, TTL), append-only additions (mid-conversation system messages), and deterministic per-message masking. Replay never edits an earlier turn client-side.
...
4. **Replay stores no provider credentials and, by default, no raw conversation content.** The ledger holds derived data: hashes, token counts, structure, labels.
...
| L2 | Server-side compaction returns blocks the client must echo back. Claude Code does not know Replay enabled it. Enabling compaction from a proxy corrupts the next turn. | High | Closed: compaction is never enabled by the proxy. Context editing is safe because it is stateless per request; compaction is not. |
...
| # | Attack | Severity | Holds? |
|---|--------|----------|--------|
| S1 | **Placeholder forgery.** Masking replaces a secret with a placeholder the model can see. Untrusted content in a tool result (a file in a cloned repository) instructs the model to write that placeholder into a shell command. Replay rehydrates the real secret into the command, which then sends it to the attacker. | Critical | Open in the current design. Resolution: rehydration is scoped. By default, placeholders rehydrate into assistant text and into file-edit tool inputs targeting paths inside the project, never into shell or network tool inputs. Every rehydration is logged with its destination. Users can widen the scope per pattern. |
...
| S3 | Replay becomes a second copy of every transcript, retained longer than the client's own. Code and secrets at rest, again. | High | Closed by constraint 4: the ledger stores hashes, token counts, structure, and labels such as file paths. Raw content storage is opt-in, encrypted at rest, with a retention setting and a purge command. |
| S4 | Provider credentials. If Replay stores or caches the API key or OAuth token, it becomes the most valuable file on the machine. | High | Closed: Replay forwards the client's authentication headers unchanged and never persists them. Headers are excluded from every log path, including debug. |
| S5 | Local dashboard renders tool output. A file in a malicious repository contains script; it executes in the dashboard with access to Replay's data. | High | Closed: terminal-only output in v0.1. When a web dashboard ships: strict escaping, a content security policy with no inline script, loopback binding, Host header check, token on every read. |
...
| P4 | Provider terms on modifying requests. | Low | Replay acts inside the user's own account on the user's own machine and adds only documented parameters. Document this and link the relevant terms. |
...
- Restricting live policies to provider-sanctioned mechanisms (L1, L2) removes the entire class of history-editing bugs and means Replay can never trip the binding check. It also means the policy catalog is small and each entry is auditable.
...
- Forwarding credentials unchanged (S4) means Replay has nothing to leak that the client did not already have.

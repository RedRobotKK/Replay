...
func runServe(args []string, stdout, stderr io.Writer) error {
	...
	freezePrefix := fs.Bool("freeze-prefix", false, "EXPERIMENTAL: pin cc_version hashes in the system prompt to a same-length constant so the cached prefix does not fork on the client's billing header, and label a tool-set epoch from the tools JSON as forwarded ("+envNoPolicy+"=1 forces off)")
	...
	if noPolicy {
		// Same rule every other live policy follows: the environment switch
		// turns them off without the operator editing a command line.
		*freezePrefix = false
	}

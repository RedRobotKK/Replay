...
// FlagSet existed. It worked, and it was invisible: docs/CLI.md is generated by
// asking each subcommand for its flags, so the reference said "mcp: Takes no
// flags" while the flag that installs the server shipped. A flag nothing can
// enumerate is a flag nobody finds.
func runMCPCommand(args []string, stdout, stderr io.Writer) error {
	fs := flag.NewFlagSet("mcp", flag.ContinueOnError)
	fs.SetOutput(stderr)
	install := fs.Bool("install", false,
		"print the MCP client configuration for this binary and exit, instead of serving")

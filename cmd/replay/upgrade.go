...
// upgradeHint is the staleness line printed under a report.
//
// It is computed from the build date already linked into this binary and asks
// the network nothing, which is the only reason it can exist: a version check
// here would be an outbound request the user did not type, and
// outbound_drift_test.go would be right to fail for it.
func upgradeHint(now time.Time) string {
	return selfupdate.StaleNotice(version.Date, now)
}
